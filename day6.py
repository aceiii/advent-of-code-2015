...
def count():
    return sum([len(filter(lambda x: x, d)) for d in lights])

def count2():
    return sum([sum(d) for d in lights])
...
for line in s.strip().split("\n"):
    words = line.split(" ")
    if words[0] == "toggle":
        a = map(int, words[1].split(","))
        b = map(int, words[3].split(","))
        toggle(a, b)
    elif words[1] == "on":
        a = map(int, words[2].split(","))
        b = map(int, words[4].split(","))
        turn_on(a,b)
    elif words[1] == "off":
        a = map(int, words[2].split(","))
        b = map(int, words[4].split(","))
...
#lights[0][0] = True
#print lights[1][0]
#print [filter(lambda x: x, d) for d in lights]
#print(count())
print(count2())
